...
def validity_time(route_list, arr_distance_matrix, arr_customer_info,total_capacity, total_time, service_time):
    ...
    for i in range(1,len(route_list)-2):
        ...
        curr_time = curr_time + dist + service_time
...
#My Change
#Change_andre
def calculate_early_delay_penalty(route_routeList_list, arr_distance_matrix, arr_customer_info, service_time):
    early_time = 0
    delay_time = 0
    ...
    for route_list in route_routeList_list: # loop over routes
        ...
        total_dist = arr_distance_matrix[0, cust]
        readyTime = arr_customer_info[cust-1, 2]
        dueTime = arr_customer_info[cust-1, 3]
        if readyTime - total_dist > 0:
            start_time = readyTime - total_dist
        else:
            start_time = 0
        #print("start:",start_time)   # ADDED MS
        ...
        for i in range(1, len(route_list) - 2):
            ...
            dueTime = arr_customer_info[cust2 - 1, 3]
...
#        print(available_customer_list_index)
        ...
        arr_nn = arr_nn[(arr_nn[:, 1] <= total_capacity) & (arr_nn[:, 12] <= total_time)]
...
def local_search_particle_wise(df_customers,arr_distance_matrix, total_time, total_capacity,  distance,  num_routes,  particle_route_list,  particle_velocity_list,  particle_position_list,  particle_distance_list, arr_customer_info, service_time):
    ...
    for i in range(len(r_customers)):
        ...
        for vehicle_no in range(num_routes-1):# LOOP2
            ...
            feasible_cust_list = df_vehicle_cust.loc[(df_vehicle_cust['readyTime']>= df_r_customers.loc[customer,'readyTime']) &
                                                     (df_vehicle_cust['readyTime'] < df_r_customers.loc[ customer, 'readyTime']) &
                                                     (df_vehicle_cust['readyTime'] > df_r_customers.loc[customer, 'dueTime']) &
                                                     (df_vehicle_cust['readyTime']<=df_r_customers.loc[customer,'dueTime'])].index.tolist()
            #get the customer's index which have a ready time>=the insertable customer's start time and a start time which is less than the due time of the insertable customer
...
#    flagged_indicies = flagged_indicies.astype(int)
    ...
    for i in range(len(flagged_indicies)):
        #print(PSO_resultlist[0][i])  # ADDED MS
        ...
        Total_Penalty = calculate_early_delay_penalty(PSO_resultlist[0][i], arr_distance_matrix, arr_customer_info, service_time)
        #print("d:",PSO_resultlist[3][i],"v:",PSO_resultlist[4][i],"p:",Total_Penalty)  # ADDED MS
        arr_results[flagged_index_val, 2] = beta*(PSO_resultlist[3][i]) + alpha*(arr_results[i,0])+ Total_Penalty
...
#    4 num_vehicles
    ...
    #print(position_resultslist[0])   # ADDED MS
    List_Penalty = []
    for i in range(len(position_resultslist[0])):
        Total_Penalty = calculate_early_delay_penalty(position_resultslist[0][i], arr_distance_matrix, arr_customer_info,service_time)
        List_Penalty.append(Total_Penalty)   # THIS WAS NOT INSIDE FOR LOOP!
    
    #print("d:",position_resultslist[3],"v:",position_resultslist[4],"p:",List_Penalty)  # ADDED MS
    
    arr_results[:,2] = beta*np.array(position_resultslist[3]) + alpha*np.array(position_resultslist[4]) + np.array(List_Penalty)  #fitness
...
#    6 particle_velocity_list
    ...
    if update_particle_indicies.size !=0: #perform update
        ...
        #print(local_search_results_list[3][update_particle_indicies])  # ADDED MS        
        Total_Penalty = calculate_early_delay_penalty(local_search_results_list[3][update_particle_indicies], arr_distance_matrix, arr_customer_info, service_time)
        ...
        #print("d:",arr_results[update_particle_indicies, 1],"v:",arr_results[update_particle_indicies, 0],"p:",Total_Penalty)  # ADDED MS
    
        
        arr_results[update_particle_indicies, 2] = beta*(arr_results[update_particle_indicies, 1])+ alpha*(arr_results[update_particle_indicies, 0]) + Total_Penalty #fitness
